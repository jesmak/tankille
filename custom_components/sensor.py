...
class CheapestSensor(CoordinatorEntity, SensorEntity):
    def __init__(self, coordinator: DataUpdateCoordinator, fuel_type: str, data: list):
        ...
        for i in range(5):
            attrs[ATTR_PRICE_ + str(i + 1)] = str(data[i][ATTR_PRICE]) if len(data) > i and data[i][
                ATTR_PRICE] is not None else None
            attrs[ATTR_UPDATED_ + str(i + 1)] = data[i][ATTR_UPDATED] if len(data) > i else None
            attrs[ATTR_STATION_ + str(i + 1)] = data[i][ATTR_STATION] if len(data) > i else None
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        for station in self.coordinator.data:
            if str(station["chain"]).lower() in ignored_chains:
                ...
                data.append({ATTR_PRICE: price["price"], ATTR_STATION: station["name"],
                             ATTR_UPDATED: datetime.fromisoformat(
                                 str(price["updated"]).removesuffix('Z')).replace(
                                 tzinfo=datetime.now().astimezone().tzinfo)})
        ...
        for i in range(5):
            self._attr_extra_state_attributes[ATTR_PRICE_ + str(i + 1)] = str(data[i][ATTR_PRICE]) if len(data) > i and \
                                                                                                      data[i][
                                                                                                          ATTR_PRICE] is not None else None
            self._attr_extra_state_attributes[ATTR_UPDATED_ + str(i + 1)] = data[i][ATTR_UPDATED] if len(
                data) > i else None
            self._attr_extra_state_attributes[ATTR_STATION_ + str(i + 1)] = data[i][ATTR_STATION] if len(
                data) > i else None

    @property
    def native_value(self):
        return self._attr_extra_state_attributes[ATTR_PRICE_ + "1"]
